...
static void app_event_handler_button_released(void);
...
void app_event_handler_on_char_requests(uint8_t access_type, sl_bt_msg_t *evt)
{
  ...
  int temp_int = 0;
  distance_monitor_feature_t *feature = NULL;
    ...
    if (((BLE_CHAR_ACCESS_TYPE_READ == access_type)
        ...
        || ((BLE_CHAR_ACCESS_TYPE_WRITE == access_type)
            && (evt->data.evt_gatt_server_user_write_request.characteristic
                == distance_monitor_features[i].char_id))) {
      // Set current feature
      feature = &distance_monitor_features[i];
      break;
    }
  }
    ...
    if (BLE_CHAR_ACCESS_TYPE_READ == access_type) {
      ...
      if (evt->data.evt_gatt_server_user_write_request.value.len
        ...
        if ((temp_int >= feature->value_min)
          ...
          if (NULL != feature->param_change_callback) {
            // Invoke value change callback function
            feature->param_change_callback();
          }
...
void app_event_handler_on_external_event(sl_bt_msg_t *evt)
{
  if (evt->data.evt_system_external_signal.extsignals
      & DISTANCE_MONITOR_TIMER_EVENT) {
    // Invoke distance monitor timer event handler to measure and
    app_logic_main_function();
  } else if (evt->data.evt_system_external_signal.extsignals
             & DISTANCE_MONITOR_BUTTON_EVENT) {
    // Invoke BTN0 event handler,  when the user pressed this button
    app_event_handler_button_released();
  } else if (evt->data.evt_system_external_signal.extsignals
             & DISTANCE_MONITOR_DISPLAY_EVENT) {
    // Invoke DISPLAY event handler
    app_logic_on_update_display();
  }
}

static void app_event_handler_button_released(void)
{
  app_log("> BTN0 Button released. Change notification status.\n");
  app_logic_configure_notification_status(true);
}

void app_event_handler_notification_status_changed(void)
{
  app_logic_configure_notification_status(false);
}
