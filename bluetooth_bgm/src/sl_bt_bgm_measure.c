...
sl_simple_timer_t bgm_report_all_timer;
...
void sli_bt_bgm_generate_record(uint16_t index)
{
  ...
  //3. base time 0xE507(year 2021), 7(month July), 5(day 5), 12 30 5(12h:30m:5s)
...
void sl_bt_bgm_measurement_notificate(uint16_t index)
{
  ...
  if (sc) {
    app_log_warning("send measurement fail in sending all records 0x%04X\n", sc);
  ...
  if (sc) {
    app_log_warning("send context fail in sending all records 0x%04X\n", sc);
...
void sl_bt_bgm_report_all_timer_cb(sl_simple_timer_t *timer, void *data)
{
  (void)data;
  (void)timer;
  ...
  if(bgm_abort_operation_flag == true){
    ...
    sc = sl_simple_timer_stop(&bgm_report_all_timer);
    return;
  }
...
void sl_bt_bgm_report_all_records(uint8_t connection)
{
  ...
  sc = sl_simple_timer_start(&bgm_report_all_timer,
                             REQUESTED_NOTIFICATION_INTERVAL,
                             sl_bt_bgm_report_all_timer_cb,
                             NULL,
                             true);
  if(sc != SL_STATUS_OK){
    app_log("bgm_report_timer failed 0x%04X\n", sc);
  }
