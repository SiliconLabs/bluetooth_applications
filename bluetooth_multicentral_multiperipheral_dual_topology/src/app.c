  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_connection_opened_id:
      ...
      if(evt->data.evt_connection_opened.master == CR_CENTRAL) {
          ...
          sc = sl_bt_gatt_discover_primary_services_by_uuid(evt->data.evt_connection_opened.connection, sizeof(serviceUUID), serviceUUID);
          app_assert_status(sc);
      ...
      /* else if connection role is PERIPHERAL ...*/
          ...
          /* update device list */
          device_list[numOfActiveConn].address      = evt->data.evt_connection_opened.address;
          device_list[numOfActiveConn].address_type = evt->data.evt_connection_opened.address_type;
          device_list[numOfActiveConn].conn_handle  = evt->data.evt_connection_opened.connection;
      ...
      /* Update device connection state. common for both master and slave roles*/
       ...
       if (numOfActiveConn == MAX_CONNECTIONS){
           ...
           sc = sl_bt_advertiser_start(advertising_set_handle, advertiser_general_discoverable, advertiser_non_connectable);
           app_assert_status(sc);
       }
       else {
    ...
    case sl_bt_evt_gatt_procedure_completed_id:
      ...
      if (discovering_service) {
        ...
        sc = sl_bt_gatt_discover_characteristics_by_uuid(evt->data.evt_gatt_procedure_completed.connection, serviceHandle, sizeof(characteristicUUID), characteristicUUID);
        app_assert_status(sc);
    ...
    case sl_bt_evt_gatt_characteristic_value_id:
      ...
      if (evt->data.evt_gatt_characteristic_value.att_opcode == gatt_handle_value_indication) {
        ...
        sc = sl_bt_gatt_server_notify_all(temp_measurement_char,evt->data.evt_gatt_characteristic_value.value.len, evt->data.evt_gatt_characteristic_value.value.data);
        app_assert_status(sc);
      }
      break;
    // -------------------------------
