...
void vl53l1x_app_process_action(void)
{
  ...
  if (is_data_ready) {
    ...
    people_count = process_people_counting_data(distance, in_zone);

    in_zone++;
    in_zone %= 2;

    sc = vl53l1x_set_roi_center(VL53L1X_ADDR, roi_center[in_zone]);
    app_assert_status(sc);
  }
}
...
{
  bool current_zone_status_is_changed = 0;
  uint8_t pattern = 0;
