...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
       ...
       * its ID is defined in gatt_configuration.btconf as "temperature_measurement".
    ...
    case sl_bt_evt_system_soft_timer_id:
      /* Measure the temperature as defined in the function measure_temperature() */
      measure_temperature();
      break;
...
static int32_t convert_to_millicelsius(int32_t adc_sample)
{
  ...
  const uint32_t CAL_TEMP0 = ((DEVINFO->CAL & _DEVINFO_CAL_TEMP_MASK)
                             >> _DEVINFO_CAL_TEMP_SHIFT);

  /* _DEVINFO_ADC0CAL3_TEMPREAD1V25_MASK is not correct in
     current CMSIS. This is a 12-bit value, not 16-bit. */
  const uint32_t CAL_VALUE0 = ((DEVINFO->ADC0CAL3 & _DEVINFO_ADC0CAL3_TEMPREAD1V25_MASK)
...
static void measure_temperature(void)
{
  ...
  uint8_t *p = htm_temperature_buffer; /* Pointer to HTM temperature buffer
                                    needed for converting values to bitstream. */
