...
static uint8_t joystick_pre_pos = 126; //~126 is the reading when the joystick is in the mid-point
...
SL_WEAK void app_init(void)
{
  ...
  sc = led_7seg_init();
  if (sc != SL_STATUS_OK) {
    sl_app_log("Warning! Failed to init led 7seg\n");
  }
  else {
    sl_app_log("Led 7seg initialized\n");
  }
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      sc = sl_bt_system_set_soft_timer(3276, SOFT_TIMER_JOYSTICK_READ, 0);
      sl_app_assert(sc == SL_STATUS_OK,
    ...
    // -------------------------------
    // This event indicates that the value of an attribute in the local GATT database
    // was changed by a remote GATT client.
    case sl_bt_evt_gatt_server_attribute_value_id:
      if(evt->data.evt_gatt_server_attribute_value.attribute
         == gattdb_7_seg_data ) {
        sc = sl_bt_gatt_server_read_attribute_value(gattdb_7_seg_data,
                                                    0,
                                                    1,
                                                    &len,
                                                    &joystick_data);
        ...
        led_7seg_display_number(joystick_data);
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      ...
      notifications_enabled = evt->data.evt_gatt_server_characteristic_status.client_config_flags;
      break;
...
static sl_status_t joystick_get_pos(uint8_t *data_pos)
{
  ...
  sc = joystick_send_command(data_pos, JOYSTICK_READ_VERTICAL_POSITION_MSB);
  return sc;
}
...
static void joystick_update_data(void)
{
  ...
  if (joystick_pre_pos != joystick_current_pos) {
    ...
    sc = sl_bt_gatt_server_write_attribute_value(gattdb_7_seg_data,
                                                 0,
                                                 1,
                                                 &joystick_data);
    sl_app_assert(sc == SL_STATUS_OK,
