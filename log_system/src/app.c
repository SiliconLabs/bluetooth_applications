...
#define EVT_CATEGORY  "[SYSTEM]: "
    case sl_bt_evt_system_boot_id:
      ...
      EVT_LOG_V(FOLLOWINGS,
                "Major = 0x%04x, Minor = 0x%04x, Patch = 0x%04x, Build = %ld, Bootloader = 0x%08x, Hw = 0x%04x, Hash = 0x%08x",
...
#define EVT_CATEGORY  "[TEST]: "
    case sl_bt_evt_test_dtm_completed_id:
      ...
      LOGN();
      EVT_LOG_I(FOLLOWINGS, "Number of Packets = %d",
    ...
    default:
      EVT_LOG_V(COMMANDS_NOT_ADDED, "Header = 0x%08x", SL_BT_MSG_ID(evt->header));
      LOGN();
      break;
  }
}
...
{
  if (direct) {
    LOG_DIRECT_ERR("0x%04X - %s", sc, msg);
  } else {
    LOGE("0x%04X - %s", sc, msg);
    LOGN();
  }
}
...
sl_status_t error_checking(sl_status_t sc, uint8_t directly)
{
  ...
  switch (sc) {
    ...
    case SL_STATUS_BT_CTRL_REMOTE_USER_TERMINATED:
      log_out(directly, "User on the remote device terminated the connection", sc);
      break;
    case SL_STATUS_BT_CTRL_REMOTE_DEVICE_TERMINATED_CONNECTION_DUE_TO_LOW_RESOURCES:
      log_out(directly, "The remote device terminated the connection because of low resources", sc);
      break;
    case SL_STATUS_BT_CTRL_REMOTE_POWERING_OFF:
      log_out(directly, "Remote Device Terminated Connection due to Power Off", sc);
      break;
    ...
    case SL_STATUS_BT_CTRL_LL_RESPONSE_TIMEOUT:
      log_out(directly, "Connection terminated due to link-layer procedure timeout", sc);
      break;
    case SL_STATUS_BT_ATT_INVALID_HANDLE:
      log_out(directly, "The attribute handle given was not valid on this server", sc);
      break;
    ...
    case SL_STATUS_BT_ATT_INSUFFICIENT_AUTHENTICATION:
      log_out(directly, "The attribute requires authentication before it can be read or written", sc);
      break;
    case SL_STATUS_BT_ATT_INVALID_OFFSET:
      log_out(directly, "Offset specified was past the end of the attribute", sc);
      break;
    case SL_STATUS_BT_ATT_ATT_NOT_FOUND:
      log_out(directly, "No attribute found within the given attribute handle range", sc);
      break;
    default:
