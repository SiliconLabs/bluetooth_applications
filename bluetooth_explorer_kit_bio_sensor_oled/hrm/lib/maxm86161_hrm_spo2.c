                              ...
                              mamx86161_hrm_data_t *hrm_data)
{
  ...
  for (i = 0; i < numSamples; i++) {
    ...
    {
  #if HRM_PS_AGC
      if ((handle->heart_rate_invalidation_previous & MAXM86161_HRM_STATUS_HRM_MASK)
                                           ...
                                           maxm86161_bpf_t *p_bpf)
{
   ...
   * check the PS DC after 1s and clear BPF data buffers with p_bpf->x[0][:] = current PS.*/
  ...
  // Look for the finger Off-to-On transition and then initialize the BPF variables
  ...
  for (i = 0; i < handle->bpf_biquads; i++) {
    ...
    new_sample_i2 = 0;
    for (j = 0; j < 3; j++) {
      // If |x| is large, scale down first before the 32-bit-int32_t multiplication.
    ...
    for (j = 1; j < 3; j++) {
      ...
      if (abs(p_bpf->yi[i][j]) < (1<<14))
        // If |y| is large, scale down first before the 32-bit-int32_t multiplication.
                                           ...
                                           mamx86161_hrm_data_t *hrm_data)
{
  ...
  int32_t hrm_ps_auto_corr_max, hrm_ps_auto_corr_current, hrm_ps_auto_corr_max_index;
  ...
  // hrm_ps_dc is the PS DC accumulator. PS is the normalized PS and also input to BPF.
  handle->hrm_ps_dc += handle->normalized_hrm_ps * handle->hrm_interpolator_factor;
  ...
  // Received new hr_update_interval samples
    ...
    *p_heartrate_invalidation = MAXM86161_HRM_STATUS_SUCCESS;    // Clears all HRM and SpO2 status bits.
    ...
    if (((handle->heart_rate_invalidation_previous & MAXM86161_HRM_STATUS_HRM_MASK) == MAXM86161_HRM_STATUS_SUCCESS)
        ...
        && (handle->hr_iframe+handle->hr_update_interval) < MAX_FRAME_SAMPLES) {
      handle->hr_iframe += handle->hr_update_interval;
      handle->num_of_lowest_hr_cycles += handle->hr_update_interval * 100 / handle->t_low0;
    }
    ...
    else if ((handle->heart_rate_invalidation_previous & MAXM86161_HRM_STATUS_HRM_MASK)
              == MAXM86161_HRM_STATUS_FINGER_ON) {
      /* Validation: Still Finger-On mode if the last frame is Finger-On
       * and the leading zeros in the current frame are 5% or more of iFrame samples. */
    ...
    if (*p_heartrate_invalidation == 0)
    {
      ...
      i = hrm_buffer_out_index;
      // Now, hrm_ps_dc is the averaged normalized PS over hr_update_interval sample (1s)
      ...
      for (k = 0; k < handle->hr_iframe - 1; k++) {
        ...
        if (m == MAX_FRAME_SAMPLES)
          m = 0;  // Wrap around
        if (((int32_t)(handle->hrm_sample_buffer[i] - hrm_ps_vpp_high * (ZR_BIAS_SCALE)) * 2 + 1)
            * ((int32_t)(handle->hrm_sample_buffer[m] - hrm_ps_vpp_high*(ZR_BIAS_SCALE)) * 2 + 1) < 0)
          zc_count++;
        if (++i == MAX_FRAME_SAMPLES)
          i = 0;  // Wrap around
      }
                                                ...
                                                mamx86161_hrm_data_t *hrm_data)
{
  ...
  // Frame process (Background job)
  // HRM_frame_process sets the flag value when a new frame of samples is available.
  if (handle->spo2->spo2_percent == SPO2_STATUS_PROCESS_SPO2_FRAME) {
    ...
    // Set the output buffer index based on the Input index.
    spo2_buffer_out_index = handle->spo2->spo2_buffer_in_index - handle->hr_iframe;
      ...
      // Validation 3: Crest factor check to exclude the finger moving and BPF transition.
        *p_heartrate_invalidation |= MAXM86161_HRM_STATUS_SPO2_CREST_FACTOR_OFF;
        ...
        if (spo2_red_ac2>0 && spo2_ir_ac2 > 0) {
          ...
          else if (handle->spo2->time_since_finger_off < 51 * handle->fs / 10 / 10) {
 ...
 *****************************************************************************/
static int32_t maxm86161_hrm_sample_process(maxm_hrm_handle_t *handle,
                                            uint32_t hrm_ps,
                                            uint32_t spo2_ps_red,
                                            uint32_t spo2_ps_ir,
                                            mamx86161_hrm_data_t *hrm_data)
