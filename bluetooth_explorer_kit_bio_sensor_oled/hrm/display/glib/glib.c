...
*
*  @param size
*  size of background data
*
*  @return
*  Returns GLIB_OK on success, or else error code
******************************************************************************/
glib_status_t glib_draw_background(glib_context_t *pContext, const uint8_t *data,
                                   uint16_t size)
{
  (void) pContext;
  if (size > dimensions.xSize * dimensions.ySize)
    return GLIB_ERROR_OUT_OF_MEMORY;

  memcpy(glib_frame_buffer, data, size);
