 ...
 **************************   STATIC FUNCTIONS   *******************************
 ******************************************************************************/
static void pulse_oximeter_timer_callback(sl_sleeptimer_timer_handle_t *timer,
                                          void *data);
...
void pulse_oximeter_init(void)
{
  ...
  // Only present SpO2 and PR
  service_data.plx_spot_check_measurement.flags = 0;
  ...
  // Only present Normal SpO2 and PR
  service_data.plx_continuous_measurement.flags = 0;
...
void pulse_oximeter_disconnect_event(sl_bt_msg_t *evt)
{
  (void)evt;
  // TODO:: Add your own code here.
  // stop sleep timer for indicate and notify
...
void pulse_oximeter_characteristic_status(sl_bt_msg_t *evt)
{
  ...
  if ((evt->data.evt_gatt_server_characteristic_status.characteristic
    ...
    if (evt->data.evt_gatt_server_characteristic_status.client_config_flags) {
      ...
      sl_sleeptimer_is_timer_running(&pulse_oximeter_timer, &timer_running);
      if (!timer_running) {
        sl_sleeptimer_start_periodic_timer_ms(&pulse_oximeter_timer,
                                              500,
                                          ...
                                          void *data)
{
  ...
  sl_bt_external_signal(PLUSE_OXIMETER_TIMER_EXT);
}
