...
sl_sleeptimer_date_t present;                  	// time structure for system time
char formattedTime[] = "";
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_connection_closed_id:
      ...
      memset(notifData, 0, NOTIF_COUNT*NOTIF_SIZE*sizeof(notifData[0][0]));				//clearing notification memory
...
void show_time()
{
  ...
  sprintf(formattedTime, "%d:%d \n%d/%d/%d\n", present.hour, present.min,
      present.month_day, ((uint8_t)present.month+1), present.year + 1900);
  ...
  sprintf(tempString, "Temp: %5.2f C", tmp_c);
 ...
 * Don't call this before initializing display
 *
 * @param[in] xOffset value to be offset on x axis while printing. positive values shift the data towards right and negative values shift them to left
 * @param[in] yOffset value to be offset on y axis while printing. positive values shift the data downwards and negative values shifts it upwards
 *****************************************************************************/
void show_notif(uint16_t xOffset, uint16_t yOffset)
{
  ...
  for (uint8_t i = notifPtr; i < NOTIF_COUNT; i++) {
    strcat(tempString, notifData[i]);
    strcat(tempString, "\n");
  }
  for (uint16_t i = 0; i < strlen(tempString); i++) {
 ...
 *
 * @param[in] uint8array that has two elements data[] and len wherein data is a pointer to uint8_t array upto 255 bytes in size.
 *****************************************************************************/
...
void dataFilter(uint8_t len, uint8_t data[])
{
  ...
  if (pData[0] != 0xAB) {                    // extension message of long notifs
    ...
    row_count = 0;
    for (uint8_t i = notifPtr; i < NOTIF_COUNT; i++) {
      row_count += sl_smartwatch_ui_char_to_rows((uint8_t) (strlen(notifData[i])));
    }
    ...
    switch (pData[4]) {                                         // command type
      ...
      case 0x93:                                                // set time
        ...
        // offset is set to zero because mobile app (DT78) sends local time
      	// month structure iterates from 0 to 11 rather than 1 to 12 as normal date format, hence - 1.
      	sc = sl_sleeptimer_build_datetime(&present, (pData[8] | pData[7] << 8),
            (pData[9] - 1), pData[10], pData[11], pData[12], pData[13], 0);
        app_assert(SL_STATUS_OK == sc, "Error while building date and time. ERR CODE: %d\n", sc);
        sc = sl_sleeptimer_set_datetime(&present);
        app_assert(SL_STATUS_OK == sc, "Error while setting date and time. ERR CODE: %d\n", sc);
        LOG("Time set successfully\n");
        break;

      case 0x91:                                        // battery level request
      ...
      case 0x72:                                                // notifications

        if (notifPtr == 0) {   // overwrite notifs when allocated memory is full
          notifPtr = NOTIF_COUNT;
        }

        switch (pData[6]) {                                     // notif type

          case 0x01:                                             // call

            notifPtr--;
            char prefix_str[] = "Call from: ";
            for (uint8_t i = 0; i < strlen(prefix_str); i++) {
              notifData[notifPtr][i] = (char) prefix_str[i];
            }
            for (uint8_t i = strlen(prefix_str);
                i < (len - 8 + strlen(prefix_str)); i++) {
              notifData[notifPtr][i] = (char) pData[i + 8 - strlen(prefix_str)];
            }
            LOG("saved CALL INFO to memory\n");
            break;

          default:                                              // general notif
            notifPtr--;
