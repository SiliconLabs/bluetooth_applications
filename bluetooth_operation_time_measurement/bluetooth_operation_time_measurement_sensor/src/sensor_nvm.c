/***************************************************************************//**
 * @file sensor_nvm.c
 * @brief accel5 sensor nvm source file.
 *******************************************************************************
 * # License
 ...
 ******************************************************************************/
void sensor_config_nvm3_init(void)
{
  Ecode_t err;
  ...
  conf_data_u32_init(NVM_OPERATION_TIME_KEY, 0, 0xFFFFFF, 1);
}

/***************************************************************************//**
 * Set NVM3 operation time.
 ******************************************************************************/
sl_status_t sensor_nvm3_set_operation_time(uint32_t operation_time)
{
  Ecode_t err;

  err = nvm3_writeData(NVM3_DEFAULT_HANDLE,
                       NVM_OPERATION_TIME_KEY,
                       &operation_time,
                       sizeof(operation_time));
  if (ECODE_NVM3_OK == err) {
    app_log("stored operation time: %lu\r\n", operation_time);
    return SL_STATUS_OK;
  } else {
    app_log("Error storing operation time\r\n");
    return SL_STATUS_FAIL;
  }
}

/***************************************************************************//**
 * Get NVM3 operation time.
 ******************************************************************************/
sl_status_t sensor_nvm3_get_operation_time(uint32_t *operation_time)
{
  Ecode_t err;

  err = conf_data_u32_read(NVM_OPERATION_TIME_KEY, operation_time);
  if (ECODE_NVM3_OK == err) {
    app_log("read operation time : %lu\r\n", *operation_time);
    return SL_STATUS_OK;
  } else {
    app_log("Error reading operation time\r\n");
    return SL_STATUS_FAIL;
  }
}
