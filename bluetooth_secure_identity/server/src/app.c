...
#include "..\common\ecdh-util.h"
#include "psa\crypto.h"
#include "sl_sleeptimer.h"

#define WDOG_FEED_TIMER 1
...
/*
 * GATT service
 * 53 69 6c 61 62 73 54 72 61 69 6e 69 6e 67 32 31 (SilabsTraining21)
 *
 * 3132676e-696e-6961-7254-7362616c6953
 * original was
 * 6c616e22-2b0b-17ec-719a-eee0497c9a09
 *
 * */
...
/*** end private data ***/
/**** private functions ******/
static sl_status_t user_read_request_cb(sl_bt_evt_gatt_server_user_read_request_t *request);
static sl_status_t gatt_server_user_write_request_cb(sl_bt_evt_gatt_server_user_write_request_t *user_write_request);

/**** end private functions ****/
uint8_t challenge[CHALLENGE_SIZE];
psa_key_id_t private_device_key = 0x8001;
mbedtls_pk_context client_root_public_key;
sl_status_t pem_to_buffer_with_size(const uint8_t * pem_str, uint8_t *out, size_t *size);


/*OK to have  remote device's root pubkey*/
static psa_key_id_t peer_device_pub_key_id,
                    server_ecdh_id,
                    share_derived_key_id;

/* re-initialize data when a connection is closed */
void data_cleanup(){
...
void feed_the_dog(uint8_t *ignore){
   (void)ignore;
   //app_log("feeding watchdog\r\n");
   //  WDOGn_Feed(WDOG(0));
}

bool check_key_initialized(psa_key_id_t key){
   ...
   /*if any of the usage, type, bits or algorithm are zero, then the key is not valid*/
...
{
  //static sl_sleeptimer_timer_handle_t wdog_feed_timer;
  //uint32_t ticks = 16384;
  //uint16_t sleeptimer_flags = 0;
  ...
  ///
   ...
   mbedtls_pk_parse_public_key(&client_root_public_key,sl_root_pub_key,sizeof(sl_root_pub_key));
   ...
   if(check_key_initialized(private_device_key) == false){
   /*  sc = import_raw_key(&private_device_key,
                           (uint8_t*)PRIVATE_KEY_BASE,
                           key_size,
                           PSA_KEY_LIFETIME_PERSISTENT); */
       /*the PEM string for the private key should be constant, but lets check
        * just to be safe*/
       /*this doesn't work if the string isn't NULL terminated*/
      key_size = strlen((char*)PRIVATE_KEY_BASE);
      sc = import_pem_key((uint8_t*)PRIVATE_KEY_BASE,key_size,
                          &private_device_key,PSA_KEY_LIFETIME_PERSISTENT);
      app_assert(sc==0,"error importing private device key %d\r\n", sc);
      memset(key_buffer,0,sizeof(key_buffer));
      /* mscStatus= MSC_WriteWord((uint32_t *)PRIVATE_KEY_BASE,key_buffer,32);*/
      mscStatus= MSC_WriteWord((uint32_t *)PRIVATE_KEY_BASE,key_buffer,247);
      if(0!=mscStatus){
      ...
      /* uncomment these to test exportability of the private key -
       * the result should be '-133'
       * sc = psa_export_key(private_device_key, key_buffer, sizeof(key_buffer),&bytes_written);
        app_log("the result of attempting to export the private key is %d\r\n", sc); */
   }
#endif

 // WDOGn_Init(WDOG(0),&wdog_init);
  /*sl_sleeptimer_start_periodic_timer(&wdog_feed_timer,
                            ticks,
                            feed_the_dog,
                            &callback_data,
                            priority,
  ...
  // app_log_debug("unknown x509 error is : %s",mbedtls_low_level_strerr(-0x262E));
...
SL_WEAK void app_process_action(void)
{
  ...
  /////////////////////////////////////////////////////////////////////////////
  ///  /* feed watchdog everytime to prove that we aren't blocked */
    //   WDOGn_Feed(WDOG(0));
}
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
   ...
   switch (SL_BT_MSG_ID(evt->header)) {
      ...
      case sl_bt_evt_system_boot_id:
         app_log("\r\n------ Silicon Labs BLE Application Layer Encryption Demo: Server Role-----\r\n");
         reset_cause = RMU_ResetCauseGet();
         if(EMU_RSTCAUSE_WDOG0==reset_cause){
            app_log("system reset by watchdog\r\n");
         }
         RMU_ResetCauseClear();

         /* recommend replacing with sleep"timer*/
         //sl_bt_system_set_soft_timer(32768,WDOG_FEED_TIMER,0);
    ...
    // This event indicates that a new connection was opened.
      ...
      case sl_bt_evt_gatt_server_user_write_request_id:
         sc = gatt_server_user_write_request_cb(&evt->data.evt_gatt_server_user_write_request);
         if(SL_STATUS_OK !=sc){
            app_log("error in gatt_server_user_write_request_cb(): %d, on handle %d\r\n", sc, evt->data.evt_gatt_server_user_write_request.characteristic);
         }
         break;
      ...
      case sl_bt_evt_gatt_server_characteristic_status_id:
         if(evt->data.evt_gatt_server_characteristic_status.characteristic == gattdb_peer_challenge){
            if(evt->data.evt_gatt_server_characteristic_status.status_flags == sl_bt_gatt_server_client_config &&
               ...
               if(SL_STATUS_OK==sc){
                  ...
                  app_log("error sending indication %d\r\n", sc);
               }
            }
         }
         break;
    ...
    ///////////////////////////////////////////////////////////////////////////
      ...
      case sl_bt_evt_sm_bonded_id:
         /*uncomment this section to print bonding info
          * app_log_info("sm_bonded event: bond handle %d and security mode %d\r\n",
...
static sl_status_t user_read_request_cb(sl_bt_evt_gatt_server_user_read_request_t *request)
{
   ...
   switch(request->characteristic){
...
#ifdef USE_CUSTOM_CERTIFICATES
         pem_to_buffer_with_size(custom_batch_certificate,buffer,&bytes_to_send);
      ...
      {
#ifdef USE_CUSTOM_CERTIFICATES
         pem_to_buffer_with_size(custom_device_certificate,buffer,&bytes_to_send);
      ...
      case gattdb_factory_cert:
         status = pem_to_buffer_with_size(custom_factory_certificate,
                                          buffer,&bytes_to_send);
         if(SL_STATUS_OK!=status){
            app_log("error parsing factory certificate %d\r\n", status);
         }
         break;
      case gattdb_root_cert:
         status = pem_to_buffer_with_size(custom_root_certificate,
                                          buffer,
                                          &bytes_to_send);
         if(SL_STATUS_OK!=status){
            app_log("error parsing root certificate %d\r\n", status);
         }
         break;
      ...
      case gattdb_server_pubkey:
      {
         ...
         if(challenge_valid == false){
            app_log("attempt to start key agreement without valid device attestation\r\n");
            return SL_STATUS_NOT_READY;
         }
         status = signPublicKey(&server_ecdh_id,&server_pubkey);
         app_assert(status==0,"failed to sign ECDH key, reason %d\r\n",
                    status);
      ...
      case gattdb_test_data:
      {
         ...
         app_assert(status==0,
                    "error from psa_aead_generate_nonce(): %d\r\n",
                    status);
         status = psa_aead_encrypt(share_derived_key_id,
                                   alg,
                                   nonce, sizeof(nonce),
                                   additional, 0,
      ...
      default:
         status = SL_STATUS_BT_ATT_INVALID_HANDLE;
  ...
  status = sl_bt_gatt_server_send_user_read_response(request->connection,
                                                     request->characteristic,
                                                     SL_STATUS_OK,
                                                     bytes_to_send,
    ...
    size_t * cert_size)
{
    ...
    * offset must attribute_value->offset-1 because we discard the first byte*/
    if(attribute_value->offset + attribute_value->value.len > CERTIFICATE_BUFFER_SIZE){
    ...
    sl_bt_evt_gatt_server_user_write_request_t *user_write_request)
{
   ...
   size_t cert_size;
   sl_status_t status = SL_STATUS_FAIL, result;
   uint32_t flags = 1;
   ...
   switch(user_write_request->characteristic){
      ...
      case gattdb_peer_device_cert:
         status = save_certificate_chunk(user_write_request,cert_buffer,&cert_size);
         if(SL_STATUS_OK == status){
            peer_device_cert_complete = true;
            mbedtls_x509_crt_init(&cert_chain);
            status = mbedtls_x509_crt_parse_der(&cert_chain,
                                                cert_buffer,
                                                cert_size);
            if(status){
               app_log("error parsing device certificate %s %s\r\n",
                       mbedtls_high_level_strerr(status), mbedtls_low_level_strerr(status));
               cert_size = 0;
               return status;
            }
         }
      ...
      case gattdb_peer_batch_cert:
         ...
         if(SL_STATUS_OK == status){
            ...
            status = mbedtls_x509_crt_parse_der(&cert_chain,cert_buffer,cert_size);
            if(status){
      ...
      case gattdb_peer_factory_cert:
         ...
         if(SL_STATUS_OK == status){
            ...
            result = mbedtls_x509_crt_parse_der(&cert_chain,cert_buffer,cert_size);
            if(result){
      ...
      case gattdb_peer_root_cert:
         ...
         if(SL_STATUS_OK == status){
            ...
            result = mbedtls_x509_crt_parse_der(&root_trust,cert_buffer,cert_size);
            if(result){
      ...
      case gattdb_client_pubkey:
      {
         ...
         status = finalize_key_agreement(server_ecdh_id,
                                         peer_device_pub_key_id,
                                         &peer_key_material,
                                         &share_derived_key_id);
         app_assert(status==0,"complete_key_agreement() returns %d\r\n", status);
      }
      break;
      case gattdb_peer_challenge_response:
      {
         ...
         app_assert(result==0,"error in verifying message %d\r\n", result);
      }
      break;

      case gattdb_device_attestation_challenge:
         app_log("signing challenge\r\n");
         status =  ble_sign_challenge(user_write_request->value.data,
                                      user_write_request->value.len,
                                      device_attestation_signature,
                                      sizeof(device_attestation_signature));

         app_assert(status==SL_STATUS_OK,
                    "result ble_sign_challenge() %d\r\n",
                    status);
         break;
      default:
   ...
   result = sl_bt_gatt_server_send_user_write_response(user_write_request->connection,
                                                       user_write_request->characteristic,
                                                       SL_STATUS_OK);
   if(peer_device_cert_complete == true && peer_batch_cert_complete &&
      peer_factory_cert_complete == true && peer_root_cert_complete == true){
      ...
      status = check_root_of_trust(&client_root_public_key,&root_trust);
      app_assert(status==0,
                 "client's root certificate does not match public key: reason %d\r\n",
                 status);
      status =  mbedtls_x509_crt_verify(&cert_chain,
                                        &root_trust,
                                        NULL,
                                        NULL,
                                        &flags,
      ...
      if (SL_STATUS_OK == status){
         ...
         status = save_peer_pub_key(&cert_chain.pk,&peer_device_pub_key_id);
         app_assert(status==0,"error saving public key from certificate, reason %d\r\n",
                    status);
         status = psa_generate_random(challenge, sizeof(challenge));
         app_assert(SL_STATUS_OK==status,"failed to generate random challenge\r\n");
      }
      else{
         app_log(" %s\r\n",mbedtls_high_level_strerr(status));
