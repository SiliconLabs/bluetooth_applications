                                   ...
                                   psa_key_id_t *derived)
{
  ...
  app_assert(status==0,"error in psa_raw_key_agreement() %d\r\n",status);
  psa_hash_compute(hash_algo,
                   shared_secret,shared_secret_bytes_written,
                   key_buffer,sizeof(key_buffer),
                   &key_bytes_written);
  app_assert(status==0,"error in psa_hash_compute() %d\r\n",status);
  ...
  status = create_import_key(derived,
                             ...
                             key_buffer,
                             sizeof(key_buffer));
  app_assert(PSA_SUCCESS==status,"error importing key %d\r\n", status);

#ifdef TEST_KEY_EXPORT
  /*try to export the derived key - this must fail! */
  status = psa_export_key(derived,export_buffer,sizeof(export_buffer),&key_bytes_written);
...
/*
 * signPublicKey
 * description: generates and signs an ECDH publid keu
 * parameters:
 *    local - id of the ECDH keypair. must be persistent since it will be used after the function returns
 ...
 sl_status_t signPublicKey(psa_key_id_t *local, struct signed_key_t *key)
{
    ...
    status = psa_export_public_key(*local,key->Q,sizeof(key->Q)+1,&output_size);
    app_assert(status==0,"psa_export_public_key(): %d\r\n",status);
    ...
    status = psa_sign_hash(private_device_key,
                  ...
                  &bytes_written);
    app_assert(PSA_SUCCESS==status,"psa_sign_hash(): %d\r\n",
               status);
    return status;
 ...
 * parameters:
 *     signed_key - pointer to a struct containing the ECDH public key and signature
 *     pubkey_from_cert - key id of the public key in the peer's device certificate
 *
 * */
                            ...
                            psa_key_id_t pubkey_from_cert)
{
   ...
   if(SL_STATUS_OK){
       app_log("peer ECDH key signature is valid\r\n");
   }
                                   ...
                                   psa_key_id_t *shared_derived_key)
{
     ...
     app_assert(status==0,"ECDH key verification failed, reason %d\r\n",
                status);
     if (SL_STATUS_OK ==status){
         status = generatePeerKeySecret(private_key_id,
                                        ...
                                        shared_derived_key);
         app_assert(status==0,"cannot generate shared secret %d\r\n",
                    status);
     }
     return status;
}
...
sl_status_t save_peer_pub_key(mbedtls_pk_context *pubkey, psa_key_id_t *pubkey_id)
{
    ...
    app_assert(status==0,"failed to load ECP group, reason %d\r\n",
               status);
    status = mbedtls_ecp_point_write_binary(&group,
                                  ...
                                  buffer,
                                  65);
    app_assert(status==0,"failed to output public key to buffer, reason %d\r\n",
               status);
    /*now import into the psa key*/
    status = create_import_key(pubkey_id,usage,type,algo,lifetime,keybits,buffer,sizeof(buffer));
    app_assert(status==0,"failed to import key, reason %d\r\n",status);
    return status;
}
                                    ...
                                    size_t *size)
{
  ...
  memcpy(out,pem.buf,pem.buflen);
  app_assert(status==0,"failed to read PEM buffer reason %d\r\n", status);
  *size = pem.buflen;
...
void print_hex(const char *message, uint8_t *block, size_t size)
{
                            ...
                            psa_key_lifetime_t lifetime)
 {
  ...
  if(strstr((const char *)pem_key_str,"PRIVATE")!=NULL){
      status = mbedtls_pk_parse_key(&pk,pem_key_str,key_size,pwd,sizeof(pwd) == 0);
      app_assert(status==0,"failed to decode private key PEM string: %d",status);
      key = mbedtls_pk_ec(pk);
      if(mbedtls_ecp_check_privkey(&p256,&key->d)==0){
        usage |= PSA_KEY_USAGE_SIGN_HASH;
        type = PSA_KEY_TYPE_ECC_KEY_PAIR(PSA_ECC_FAMILY_SECP_R1);
        bytes_to_import = 32;
        mbedtls_mpi_write_binary(&key->d,buffer,bytes_to_import);
      }
  }
  else if(strstr((const char *)pem_key_str,"PUBLIC")!=NULL){
      status = mbedtls_pk_parse_public_key(&pk,pem_key_str,key_size);
      app_assert(status==0,"%s\r\n", mbedtls_high_level_strerr(status));
      key = mbedtls_pk_ec(pk);
      if(mbedtls_ecp_check_pubkey(&p256,&key->Q) == 0){
...
sl_status_t check_root_of_trust(mbedtls_pk_context *root_public_key,
                                mbedtls_x509_crt *root)
 {
   ...
   if(PSA_SUCCESS!= status){
       app_log("error hashing tbs : %d\r\n", status);
   }

   /*verify the hash using the preprovisioned key */
   status= mbedtls_pk_verify(root_public_key,MBEDTLS_MD_SHA256,
                             hash,sizeof(hash),
                             root->sig.p,root->sig.len);
   return status;
 }
