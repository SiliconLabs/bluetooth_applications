 ...
 ******************************************************************************/
sl_status_t verify_signature_remote(uint8_t *buffer, size_t bufsize, uint8_t *sig_buffer, size_t size)
{
  sl_status_t status;
  ...
  if(status){
      app_log_debug("validate public key: %02X\r\n", status);
  }
  // Verify signature
  //print_error_cycle(
