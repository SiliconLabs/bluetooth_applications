...
#include "consumer_app.h"
#include "consumer_nvm.h"
#include "mikroe_cmt_8540s_smt.h"
#include "app.h"

#define CONSUMER_BUTTON_EVENT                       1
...
static char dev_name[] = "DM_SENDER";

// Connection state handle.
static conn_state_t conn_state = booting;

// Dosimeter sender service handle.
static uint32_t service_handle;

// Radiation characteristic handle.
static uint16_t characteristic_handle;

// Dosimeter sender service UUID 128.
static const uint8_t dosimeter_sender_service[16] = {
  0xdf, 0x6a, 0xb2, 0x20, 0xe1, 0x71, 0x7b, 0xa5,
  0x52, 0x43, 0x70, 0xa5, 0xc4, 0xe6, 0x84, 0x80
};

// Radiation characteristic UUID 128.
static const uint8_t radiation_characteristic[16] = {
  0x51, 0xb9, 0xd7, 0xa5, 0x76, 0xa7, 0xa7, 0xb8,
  0x36, 0x43, 0xbb, 0xa9, 0xc2, 0x59, 0x8a, 0x0e
};

// Sender address.
static const bd_addr sender_address = {
  .addr = { 203, 121, 201, 35, 164, 96 },
};

// Buzzer active status handle.
static bool buzzer_active;

// Struct that holds consumer configuration.
static consumer_nvm_config_t cfg;

// Timer handle.
static sl_sleeptimer_timer_handle_t my_timer;

// Connection handle.
static uint8_t sender_connection_handle;
...
static void consumer_gatt_service(sl_bt_msg_t *evt);

// Characteristic found handler.
static void consumer_gatt_characteristic(sl_bt_msg_t *evt);

// Callback for configuration timeout.
static void timer_callback(sl_sleeptimer_timer_handle_t *handle, void *data);

// External event handler.
static void consumer_external_event_handle();
...
{
  app_log("BLE Consumer\r\n");
  consumer_app_init();
  consumer_nvm3_get_config(&cfg);

  // Load config from nvm.
  app_log("Conusmer configuration\r\n");
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_external_signal_id:
      consumer_external_event_handle();
      break;

    case sl_bt_evt_gatt_service_id:
      consumer_gatt_service(evt);
      break;
    ...
    case sl_bt_evt_gatt_characteristic_id:
      consumer_gatt_characteristic(evt);
      break;
...
static void scanner_legacy_advertisement_report_handle(sl_bt_msg_t *evt)
{
  ...
  // If a sender advertisement is found...
...
void system_boot_handle(sl_bt_msg_t *evt)
{
  ...
  if (sl_simple_button_get_state(&sl_button_btn0)) {
    ...
    consumer_oled_app_config_mode_display();
    ...
    app_assert_status(sc);
    app_log("Start scanning sender\r\n");
    conn_state = scanning;
    consumer_oled_app_disconnected_display();
  }
}
...
void connection_open_handle(sl_bt_msg_t *evt)
{
  ...
  if (!memcmp(&sender_address, &evt->data.evt_connection_opened.address, 6)) {
    app_log("Connected to sender\r\n");
    consumer_oled_app_radiation_display(0);
    ...
    sender_connection_handle = evt->data.evt_connection_opened.connection;

    // Find sender's service after connecting to sender.
    sc = sl_bt_gatt_discover_primary_services_by_uuid(sender_connection_handle,
                                                      16,
                                                      dosimeter_sender_service);
    app_assert_status(sc);
  } else {
...
void connection_close_handle(sl_bt_msg_t *evt)
{
  ...
  if (evt->data.evt_connection_closed.connection == sender_connection_handle) {
    consumer_oled_app_disconnected_display();
  }
  (void)evt;
...
void receive_notification(sl_bt_msg_t *evt)
{
  ...
  consumer_oled_app_radiation_display((float)recv_data);
  ...
  switch (evt->data.evt_gatt_server_user_write_request.characteristic) {
    case gattdb_alarm_threshold:
      if (evt->data.evt_gatt_server_user_write_request.value.len > 10) {
        ...
        consumer_nvm3_set_alarm_threshold(threshold);
        sc = SL_STATUS_OK;
      }
      break;
    ...
    case gattdb_notification_status:
      if (evt->data.evt_gatt_server_user_write_request.value.len != 1) {
        ...
        consumer_nvm3_set_notification_status((bool)value);
        sc = SL_STATUS_OK;
      }
      break;
    ...
    case gattdb_click_noise_status:
      if (evt->data.evt_gatt_server_user_write_request.value.len != 1) {
        ...
        consumer_nvm3_set_click_noise_status((bool)value);
        sc = SL_STATUS_OK;
      }
      break;
...
void user_read_request_handle(sl_bt_msg_t *evt)
{
  ...
  switch (evt->data.evt_gatt_server_user_read_request.characteristic) {
    case gattdb_alarm_threshold:
      threshold = consumer_nvm3_get_alarm_threshold();
    ...
    case gattdb_notification_status:
      status = consumer_nvm3_get_notification_status();
    ...
    case gattdb_click_noise_status:
      status = consumer_nvm3_get_click_noise_status();
 ...
 *****************************************************************************/
void consumer_gatt_service(sl_bt_msg_t *evt)
{
  (void)&evt;
 ...
 *****************************************************************************/
void consumer_gatt_characteristic(sl_bt_msg_t *evt)
{
  (void)&evt;
...
void comsumer_bt_evt_gatt_procedure_completed(sl_bt_msg_t *evt)
{
  ...
  if (conn_state == discover_services) {
    // Discover sender characteristic on the responder device
    sc = sl_bt_gatt_discover_characteristics_by_uuid(sender_connection_handle,
                                                     service_handle,
                                                     16,
  ...
  // If characteristic discovery finished, enable indications
    ...
    sc = sl_bt_gatt_set_characteristic_notification(sender_connection_handle,
 ...
 *****************************************************************************/
void consumer_external_event_handle()
{
  if (!cfg.notification_status) {
    consumer_nvm3_set_notification_status(true);
    cfg.notification_status = true;
  } else {
    consumer_nvm3_set_notification_status(false);
  ...
  if (sl_button_get_state(handle) == SL_SIMPLE_BUTTON_RELEASED) {
    if (&sl_button_btn0 == handle) {
      if (conn_state != booting) {
        sl_bt_external_signal(CONSUMER_BUTTON_EVENT);
      }
    }
  }
}
