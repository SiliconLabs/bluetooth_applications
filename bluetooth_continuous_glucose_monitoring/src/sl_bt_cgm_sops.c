...
void sl_bt_cgm_set_comm_interval(uint8_t interval)
{
  ...
  if (interval == 0) {
    ...
    if (session_started == true) {
      ...
      sc = sl_sleeptimer_stop_timer(&cgm_periodic_timer);
      if (sc != SL_STATUS_OK) {
...
void sl_bt_cgm_get_cal_value(sl_bt_msg_t *evt)
{
  ...
  uint16_t index = (uint16_t)(array[1] | array[2] << 8);
...
void sl_bt_cgm_set_cal_value(sl_bt_msg_t *evt)
{
  ...
  uint8_t *array = evt->data.evt_gatt_server_attribute_value.value.data;
  uint16_t record_num = (uint16_t)(array[8] | array[9] << 8);
  calibration_value[record_num].concentration =
    (uint16_t)(array[1] | array[2] << 8);
  calibration_value[record_num].cal_time = (uint16_t)(array[3] | array[4] << 8);
  calibration_value[record_num].sample_location = array[5];
  calibration_value[record_num].next_cal = (uint16_t)(array[6] | array[7] << 8);
  calibration_value[record_num].record_num =
    (uint16_t)(array[8] | array[9] << 8);
...
{
  uint8_t *array = evt->data.evt_gatt_server_attribute_value.value.data;
  uint16_t high_alert_value = (uint16_t)(array[1] | array[2] << 8);
  // 4.13.3.7
...
{
  uint8_t *array = evt->data.evt_gatt_server_attribute_value.value.data;
  uint16_t low_alert_value = (uint16_t)(array[1] | array[2] << 8);
  // 4.15.3.10
...
{
  uint8_t *array = evt->data.evt_gatt_server_attribute_value.value.data;
  uint16_t hypo_alert_level = (uint16_t)(array[1] | array[2] << 8);
  // 4.15.3.13
...
{
  uint8_t *array = evt->data.evt_gatt_server_attribute_value.value.data;
  uint16_t hyper_alert_level = (uint16_t)(array[1] | array[2] << 8);
...
{
  uint8_t *array = evt->data.evt_gatt_server_attribute_value.value.data;
  uint16_t level = (uint16_t)(array[1] | array[2] << 8);
  if (level > 100) {
