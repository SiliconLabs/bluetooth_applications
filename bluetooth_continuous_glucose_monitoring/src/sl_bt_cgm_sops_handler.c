...
void sl_bt_cgm_handle_sops(sl_bt_msg_t *evt)
{
  ...
  if (SL_BT_CGM_E2E_CRC_SUPPORTED) {
    crc = (uint16_t)(array[len - 2] | array[len - 1] << 8);
  ...
  switch (opcode) {
    ...
    case START_THE_SESSION:
      ...
      if (session_started == false) {
        sc = sl_sleeptimer_start_periodic_timer(&cgm_periodic_timer,
                                                ...
                                                0);
        if (sc != SL_STATUS_OK) {
          app_log("sl_sleeptimer_start_periodic_timer failed 0x%04X\n",
                  (unsigned int)sc);
        }
    ...
    case GET_CGM_COMM_INTEVAL:
      ...
      break;
    // CGMS/SEN/CGMCP/BV-08-C [CGM Specific Ops – ‘Set Glucose Calibration
    //   value’]
    case SET_CAL_VALUE:
