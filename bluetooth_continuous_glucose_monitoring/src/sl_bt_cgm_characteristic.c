 ...
 * */
static uint8_t session_start_time[11] =
{ 0xE5, 0x07, 0x07, 0x05, 0x12, 0x30, 0x05, \
  0x08, 0x00, 0x00, 0x00 };

uint8_t feature[FEATURE_LEN] = { 0x00 };

/*
 * The CGM Session Run Time shall define the expected run time of the CGM
 *   session.
 * The Typically CGM Sensors have a limited run time which they are approved
 *   for.
...
void sl_bt_cgm_init_database(void)
{
  ...
  if (SL_BT_CGM_E2E_CRC_SUPPORTED) {
    feature[1] = feature[1] | (uint8_t)SL_BT_CGM_E2E_CRC;
  ...
  if (SL_BT_CGM_MULTI_BOND_SUPPORTED) {
    feature[1] = feature[1] | (uint8_t)SL_BT_CGM_MULTI_BOND;
  }
  if (SL_BT_CGM_MULTI_SESSION_SUPPORTED) {
    feature[1] = feature[1] | (uint8_t)SL_BT_CGM_MULTI_SESSION;
  }
...
void sl_bt_cgm_handle_sst_write(sl_bt_msg_t *evt)
{
  ...
  if ((SL_BT_CGM_E2E_CRC_SUPPORTED) && (len != SL_BT_CGM_SST_WITHCRC_LEN)) {
